...
void adjuest3(vector<int>&res, int root, int n)
{
    int temp = res[root], j = 2*root+1;
    while(j<n)
    {
 ...
 * 遍历根节点，然后设置左叶子节点，判断左叶子节点是不是超过数组范围，
 * 要是没有超过，那么在左叶子和右叶子中选择一个最大的节点作为
 * 可能要置换的节点，判断当前根节点是不是大于这个最大的叶子节点，
 * 如果大于的话，那么不用置换，退出当前循环，否则，根节点的值赋值为
 * 当前的最大的叶子节点的值即可，叶子节点的值继续往下遍历，知道最后一层的叶子节点。
 * 
 * 当最大堆的结构完成之后，需要交换根节点和最后一个节点，
 * 然后交换之后根节点不满足最大堆的性质，重新进行排序构建最大堆，这个时候
