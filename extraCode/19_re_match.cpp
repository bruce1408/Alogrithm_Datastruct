...
// {
	...
	// {
		// if(pattern[nump]==str[nums] || (pattern[nump]=='.' && nums!=str.size()-1))
			// return matchCore(str,nums+1,pattern,nump+2) || matchCore(str,nums+1,pattern,nump)||matchCore(str,nums,pattern,nump+2);
		// else
...
bool matchCore(const char* str, const char*pattern)
{
	...
	if(*(pattern+1)=='*')//下一个元素是‘*’，那么就判断前一个元素，如果相等，字符串要向后移动一次。
	{
